          ...
          if (type === 'personalizado') {
            container.innerHTML = `
              <div class="mb-3">
                ...
                <input id="customNumber" name="recipients" type="tel" class="form-control"
                ...
                html += `
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="recipients"
                      value="${item.phone}" id="r_${item.phone}" checked>
                    <label class="form-check-label" for="r_${item.phone}">
                      ${item.phone}${item.producto ? ' – ' + item.producto : ''}
                    </label>
                  </div>`;
              });
...
router.get('/list', function(req, res) {
  ...
  query.then(items => {
    const list = items
      .map(c => ({ phone: c.numero || c.telefono, producto: Array.isArray(c.garantias) && c.garantias.length ? c.garantias[0].producto : '' }))
      .filter(x => x.phone);
    res.json(list);
  }).catch(() => res.json([]));
});
...
router.post('/', function(req, res) {
  ...
  function sendNext() {
    ...
    setTimeout(() => {
      const promise = imageUrl ? whatsappConfig.sendWhatsAppMedia(phone, imageUrl, message)
                                : whatsappConfig.sendWhatsAppMessage(phone, message);
      promise.then(() => { results.push({ phone, success: true }); idx++; sendNext(); })
